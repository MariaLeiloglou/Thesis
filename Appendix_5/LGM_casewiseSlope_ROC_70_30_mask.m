...
%load the files one by one 
 for q = 1:length(mat) % for each one of the files
     ...
     validmalignantnumber=round(malignantnumber/3.33);%100/30->number of 30% of tumour data
     validbenignnumber=round(benignnumber/3.33);%100/30->number of 30% of healthy data
     ...
     %truth
     score_log1=predict(mdl,predvalidation);%apply trained model to predict responses for the new validation data
