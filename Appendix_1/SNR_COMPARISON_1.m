...
% two times for the two systems


promptk='How many trials/images for each method (up to 15)?';%here we have two different methods of filtration
h=input(promptk); 
S=zeros(2,h);
B=zeros(2,h);
Flsd=zeros(2,h);
...
for i=1:2
    ...
    for j=1:h
        ...
        [c2,r2,~]=impixel(im);
        close figure 2
        FluorT=im(r1:r1+windo-1,c1:c1+windo-1);
        NonFluorT=im(r2:r2+windo-1,c2:c2+windo-1);
        ...
        S(i,j)=mean(FluorT);
        B(i,j)=mean(NonFluorT);
        Flsd(i,j)=std(FluorT);
        sd(i,j)=std(NonFluorT);
    end
end
difference=S-B;
SNRs=difference./sd;
...
save('meat_methanol_stock_425_625vs500_625.mat','Fcrit','Ftest','tcrit','ttest','SNR1mean','SD1','SNR2mean','SD2');